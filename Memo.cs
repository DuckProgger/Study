...
namespace Generic // Обобщения
{
    class Gen<T> // объявление обощённого класса. Т - параметр типа (Gen<T> - открыто сконструированный тип)
    {
        T ob; // объявить переменную типа Т

        public Gen(T o) // конструктор обобщённого класса
        {
            ob = o; // при создании объекта тип переменной o будет известен и его можно присвоить переменной ob того же типа
        }

        public T GetOb() // с помощью этого метода можно получить значение ob
        {
            return ob;
        }
    }
    ...
    class GenDemo
    {
        void Demo()
        {
            Gen<int> iOb;  // создание варианта класса Gen<T> для типа int (Gen<int> - закрыто сконструированный тип). int - параметр типа
            iOb = new Gen<int>(120); // присваивание ссылки на экземпляр объекта класса Gen
            int v = iOb.GetOb(); // получить значение ob

            Gen<string> sOb;  // создание варианта класса Gen<T> для типа string 
            sOb = new Gen<string>("Текст"); // присваивание ссылки на экземпляр объекта класса Gen
            string str = sOb.GetOb();

            //iOb = sOb; ошибка - Gen<int> != Get<string>

            //обобщения автоматически обеспечивают типовую безопасность всех операций, 
            //затрагивающих класс Gen.В ходе выполнения этих операций
            //обобщения исключают необходимость обращаться к приведению типов и проверять
            //соответствие типов в коде вручную.
            //По существу, обобщения переводят ошибки при выполнении в разряд ошибок при компиляции.
    ...
    class LimitedClass<T> where T : A // в качестве параметра типа при создании экземпляра можно использовать
                                 // только класс, базовым которого является класс А (или сам класс А)
    {
